...
# 인쇄(Print) 시 불필요한 UI(버튼, 입력창 등)를 숨기고 깔끔하게 출력되도록 하는 CSS 스타일 적용
st.markdown(
    """
    <style>
    @media print {
        /* 인쇄 시 숨길 요소들: 상단바, 사이드바, 입력 폼, 버튼 등 */
...
with st.form("expert_quiz_form"):
  grade = st.text_input(
      "학년을 입력하세요 (예: 초등학교 3학년, 중학교 2학년)", value="중학교 2학년"
  )
  book_title = st.text_input(
      "책 제목을 입력하세요 (예: 난쟁이가 쏘아 올린 작은 공)", value=""
  )
  author_name = st.text_input(
      "작가 이름을 입력하세요 (한글 또는 영어, 예: 조세희 / Cho Se-hui)", value=""
  )
...
2. 정답지 섹션 시작할 때: [정답지] 라고 적어주세요. (여기에는 각 문제의 정답, 상세 해설, 서술형 모범 답안 및 채점 기준을 포함하세요)
      ...
      user_prompt = (
          f"대상 학년: {grade}\n책 제목: {book_title}\n작가 이름:"
          f" {author_name if author_name else '입력 안"
          " 함'}\n\n[문제지]와 [정답지]를 명확히 구분하여 객관식 3문제(5지선다)와"
          " 서술형 2문제를 출제해주세요."
      )

      with st.spinner(
          "전문가 페르소나가 책과 작가 정보를 분석하고 고품질 문항을 출제하는"
          " 중입니다..."
      ):
        # 안정적인 Gemini 모델 지정
      ...
      if "[문제지]" in full_result and "[정답지]" in full_result:
        ...
        answer_part = (
            "정답지를 분리하는 중 형식이 일부 일치하지 않았습니다. 전체"
            " 내용을 참고해주세요."
        )
...
if st.session_state.get("generated", False):
  ...
  st.markdown(
      "<h2 style='text-align: center; color: #FF4B4B;'>📖 책의 온도 - 읽은"
      " 만큼 성장합니다.</h2>",
      unsafe_allow_html=True,
  )
  st.markdown("---")
